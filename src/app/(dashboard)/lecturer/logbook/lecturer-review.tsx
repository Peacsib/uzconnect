...
import { approveLogbookLecturer } from "@/lib/actions/logbook"
import { StatusBadge } from "@/components/common/status-badge"
import { formatDate } from "@/lib/utils"
import { BookOpen, CheckCircle, MessageSquare, User, Building2, Calendar, ShieldCheck } from "lucide-react"

export function LecturerLogbookReview({ entries }: { entries: any[] }) {
  ...
  async function handleApprove() {
    ...
    setLoading(true)
    try {
      await approveLogbookLecturer(activeEntry.id, comment)
      setSuccess(true)
      activeEntry.lecturerApproved = true
      activeEntry.lecturerComment = comment
  ...
  if (entries.length === 0) {
    return (
      <div className="bg-card rounded-xl border p-12 text-center max-w-md mx-auto space-y-3">
        ...
        <p className="font-medium">No Logbook Entries Awaiting Review</p>
        <p className="text-xs text-muted-foreground">
          Student weekly entries will appear here as soon as they are submitted.
        </p>
      </div>
    )
  }
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  setComment(entry.lecturerComment || "")
                }}
        ...
        {activeEntry && (
          <div className="bg-card rounded-xl border p-6 shadow-sm space-y-6">
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="p-4 rounded-lg bg-muted/40 border space-y-1">
                <span className="font-semibold text-xs text-primary block">Actual Tasks & Deliverables:</span>
                <p className="text-foreground whitespace-pre-wrap">{activeEntry.actualTasks}</p>
              </div>

              <div className="p-4 rounded-lg bg-muted/40 border space-y-1">
                <span className="font-semibold text-xs text-primary block">Student Reflection & Learning:</span>
                <p className="text-foreground italic">{activeEntry.reflection}</p>
              </div>
            </div>

            {/* Workplace Supervisor feedback if any */}
            {activeEntry.supervisorComment && (
              <div className="p-4 rounded-lg bg-primary/5 border border-primary/20 space-y-1 text-xs">
                <div className="flex items-center gap-1.5 font-semibold text-primary">
                  <ShieldCheck size={14} />
                  <span>Workplace Supervisor Note:</span>
                </div>
                <p className="italic text-foreground">"{activeEntry.supervisorComment}"</p>
              </div>
            )}
            ...
            <div className="border-t pt-5 space-y-4">
              <h4 className="font-semibold text-sm flex items-center gap-2">
                ...
                <span>Academic Feedback & Verification</span>
              </h4>
              ...
              {success && (
                <div className="p-3 bg-green-500/10 border border-green-500/20 text-green-700 dark:text-green-400 rounded-lg text-xs flex items-center gap-2">
                  <CheckCircle size={14} />
                  <span>Logbook approved and feedback recorded successfully!</span>
                </div>
              )}

              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-foreground">Lecturer Comments / Academic Advice</label>
                <textarea
                  rows={3}
                  value={comment}
                  onChange={(e) => setComment(e.target.value)}
                  placeholder="Provide academic advice, recommendations, or note alignment with degree objectives..."
              ...
              <div className="flex items-center justify-end">
                ...
                >
                  <CheckCircle size={16} />
                  <span>{loading ? "Approving..." : "Approve & Sign Off Entry"}</span>
                </button>
              </div>
            </div>
          </div>
