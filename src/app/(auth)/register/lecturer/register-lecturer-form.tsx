...
const schema = z.object({
  name: z.string().min(2, "Full title and name required"),
...
export function RegisterLecturerForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {[
        { name: "name", label: "Full Name & Title", placeholder: "Dr. Farai Makudza" },
        { name: "email", label: "University Email", placeholder: "f.makudza@uz.ac.zw" },
        { name: "password", label: "Password", placeholder: "Min 8 characters", type: "password" },
        { name: "departmentName", label: "Department", placeholder: "Department of Computer Science" },
      ...
      <button
        ...
        className="w-full py-2.5 px-4 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 transition disabled:opacity-60 flex items-center justify-center gap-2"
      >
        {isSubmitting && <Loader2 size={16} className="animate-spin" />}
        {isSubmitting ? "Submitting..." : "Register as Lecturer"}
      </button>
